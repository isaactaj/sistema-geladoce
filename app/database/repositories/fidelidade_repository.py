# app/database/repositories/fidelidade_repository.py
...
class FidelidadeRepository:
    """
    Camada de persistência da fidelidade.

    Responsabilidades:
    - calcular pontos pela RN05
    - aplicar movimentações de pontos
    - registrar histórico em mov_fidelidade
    - consultar extrato / saldo atual do cliente

    Observação importante:
    - NÃO altera o campo clientes.status, pois no projeto ele já está sendo
      usado para status cadastral / exclusão lógica.
    """
    ...
    def calcular_pontos_rn05(self, tipo_cliente, valor_total) -> int:
        """
        RN05:
        - Varejo: 1 ponto a cada R$ 5,00
        - Revendedor: 2 pontos a cada R$ 50,00
        """
    ...
    def listar_movimentacoes(
        ...
        if cliente_id is not None:
            where_parts.append("m.cliente_id = %s")
            params.append(int(cliente_id))

        if venda_id is not None:
            where_parts.append("m.venda_id = %s")
            params.append(int(venda_id))

        if where_parts:
            sql += " WHERE " + " AND ".join(where_parts)

        sql += " ORDER BY m.data DESC, m.id DESC LIMIT %s"
    ...
    def movimentar_fidelidade(
        ...
        """
        Aplica a movimentação e registra histórico.

        Regras:
        - ADICIONAR: soma em pontos_atuais e total_acumulado
        - REMOVER: remove até zerar (não fica negativo)
        - RESGATAR: exige saldo suficiente
        - BONUS: soma em pontos_atuais e total_acumulado
        - ZERAR: zera pontos_atuais
        """
